...
{ 
    public class OwinDataSourceMiddleware
    {
        readonly AppFunc _next;
        readonly bool _threadSafeItem;

        public OwinDataSourceMiddleware(AppFunc next)
            : this(next, threadSafeItem: false)
        {

        }

        public OwinDataSourceMiddleware(AppFunc next, bool threadSafeItem)
        {
            _next = next;
            _threadSafeItem = threadSafeItem;
        }

        public async Task Invoke(IDictionary<string, object> environment)
        {
            var scopeContext = new OwinRequestScopeContext(environment, _threadSafeItem);
